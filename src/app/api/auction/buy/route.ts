...
export async function POST(request: NextRequest) {
  try {
    ...
    // Start a transaction if replica sets are supported,
    // but for simple local MongoDB standalone setup, we will use mongoose updates.
    // Let's first fetch both documents.
    ...
    // Perform updates using a MongoDB session to ensure consistency if possible,
    // or run them sequentially with error rollback.
